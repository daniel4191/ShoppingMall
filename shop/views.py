...
def show_category(request, category_id):
    ...
    lank_products = Product.objects.filter(
        category=category).order_by('-hit')[:4]
...
def product_detail(request, pk):
    ...
    context = {"quantity_list": quantity_list, "product": product,
               "point": point, "category": category, "categories": categories}
...
def cart_or_buy(request, pk):
    ...
    initial = {'name': product.name,
               'amount': product.price, 'quantity': quantity}
    cart = Cart.objects.filter(user=user)
    if request.method == 'POST':
        if 'add_cart' in request.POST:
            for i in cart:
                if i.products == product:
                    product = Product.objects.filter(pk=pk)
                    Cart.objects.filter(user=user, products__in=product).update(
